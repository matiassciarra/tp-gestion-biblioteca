...
export const CreateLibro = async (req, res) => {
    ...
    try {
        ...
        if (!genero) {
            return res.status(400).json({ error: "El ID del género no existe." });
        }

        // Verificar si el autor existe
        const autor = await Autor.findByPk(id_autor);
        if (!autor) {
            return res.status(400).json({ error: "El ID del autor no existe." });
        }
...
export const updateLibro = async(req,res)=>{
    ...
    try {
        ...
        await libro.update({
            ...
            fecha_publicacion: fecha_publicacion !== undefined ? new Date(fecha_publicacion) : libro.fecha_publicacion,
            url: url !== undefined ? url : libro.url,
            estado_libro: estado_libro !== undefined ? estado_libro : libro.estado_libro,
        });
